...
// 滤波--点云索引过滤
template<typename PointT>
void PCLUTILS::filterIndices(const typename pcl::PointCloud<PointT>::Ptr& cloud_in, typename pcl::PointCloud<PointT>::Ptr& cloud_out, pcl::PointIndices::Ptr &indices, bool is_negative){
    filter::extractIndices<PointT>(cloud_in, cloud_out, indices, is_negative);  // 调用filter.hpp中的函数
}
template void PCLUTILS::filterIndices<pcl::PointXYZ>(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_out, pcl::PointIndices::Ptr &indices, bool is_negative);
template void PCLUTILS::filterIndices<pcl::PointXYZI>(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_out, pcl::PointIndices::Ptr &indices, bool is_negative);
template void PCLUTILS::filterIndices<pcl::PointXYZRGB>(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_out, pcl::PointIndices::Ptr &indices, bool is_negative);
template void PCLUTILS::filterIndices<pcl::PointNormal>(const pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_in, pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_out, pcl::PointIndices::Ptr &indices, bool is_negative);
...
// 滤波--设置滤波条件
template<typename PointT>
typename pcl::FieldComparison<PointT>::Ptr PCLUTILS::filterCondition(const std::string &field_name, int val, const std::string &comparison){
    return filter::removalCondition<PointT>(field_name, val, comparison);  // 调用filter.hpp中的函数
}
template pcl::FieldComparison<pcl::PointXYZ>::Ptr PCLUTILS::filterCondition<pcl::PointXYZ>(const std::string &field_name, int val, const std::string &comparison);
template pcl::FieldComparison<pcl::PointXYZI>::Ptr PCLUTILS::filterCondition<pcl::PointXYZI>(const std::string &field_name, int val, const std::string &comparison);
template pcl::FieldComparison<pcl::PointXYZRGB>::Ptr PCLUTILS::filterCondition<pcl::PointXYZRGB>(const std::string &field_name, int val, const std::string &comparison);
template pcl::FieldComparison<pcl::PointNormal>::Ptr PCLUTILS::filterCondition<pcl::PointNormal>(const std::string &field_name, int val, const std::string &comparison);

// 滤波--设置滤波条件
template<typename PointT>
typename pcl::FieldComparison<PointT>::Ptr PCLUTILS::filterCondition(const std::string &field_name, float val, const std::string &comparison){
    return filter::removalCondition<PointT>(field_name, val, comparison);  // 调用filter.hpp中的函数
}
template pcl::FieldComparison<pcl::PointXYZ>::Ptr PCLUTILS::filterCondition<pcl::PointXYZ>(const std::string &field_name, float val, const std::string &comparison);
template pcl::FieldComparison<pcl::PointXYZI>::Ptr PCLUTILS::filterCondition<pcl::PointXYZI>(const std::string &field_name, float val, const std::string &comparison);
template pcl::FieldComparison<pcl::PointXYZRGB>::Ptr PCLUTILS::filterCondition<pcl::PointXYZRGB>(const std::string &field_name, float val, const std::string &comparison);
template pcl::FieldComparison<pcl::PointNormal>::Ptr PCLUTILS::filterCondition<pcl::PointNormal>(const std::string &field_name, float val, const std::string &comparison);
 

// 滤波--条件滤波
template<typename PointT>
void PCLUTILS::filterConditionRemovalFilter(const typename pcl::PointCloud<PointT>::Ptr &cloud_in, 
    ...
    filter::conditionRemovalFilter<PointT>(cloud_in, cloud_out, cond_list, cond_op, is_negative);  // 调用filter.hpp中的函数
}
template void PCLUTILS::filterConditionRemovalFilter<pcl::PointXYZ>(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_in, 
    pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_out,
    const typename std::vector<pcl::FieldComparison<pcl::PointXYZ>::Ptr> &cond_list, 
    const std::string &cond_op, bool is_negative); 
template void PCLUTILS::filterConditionRemovalFilter<pcl::PointXYZI>(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_in, 
    pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_out, 
    const typename std::vector<pcl::FieldComparison<pcl::PointXYZI>::Ptr> &cond_list, 
    const std::string &cond_op, bool is_negative); 
template void PCLUTILS::filterConditionRemovalFilter<pcl::PointXYZRGB>(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud_in, 
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud_out, 
    const typename std::vector<pcl::FieldComparison<pcl::PointXYZRGB>::Ptr> &cond_list, 
    const std::string &cond_op, bool is_negative); 
template void PCLUTILS::filterConditionRemovalFilter<pcl::PointNormal>(const pcl::PointCloud<pcl::PointNormal>::Ptr &cloud_in, 
    pcl::PointCloud<pcl::PointNormal>::Ptr &cloud_out,
    const typename std::vector<pcl::FieldComparison<pcl::PointNormal>::Ptr> &cond_list,
    const std::string &cond_op, bool is_negative);
...
// 滤波--直通滤波
template<typename PointT>
void PCLUTILS::filterPassthrough(const typename pcl::PointCloud<PointT>::Ptr& cloud_in, typename pcl::PointCloud<PointT>::Ptr& cloud_out,const std::string& axis, float min, float max){
    filter::passThroughFilter<PointT>(cloud_in, cloud_out, axis, min, max);  // 调用filter.hpp中的函数
}
template void PCLUTILS::filterPassthrough<pcl::PointXYZ>(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_out,const std::string& axis, float min, float max);    
template void PCLUTILS::filterPassthrough<pcl::PointXYZI>(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_out,const std::string& axis, float min, float max);
template void PCLUTILS::filterPassthrough<pcl::PointXYZRGB>(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_out,const std::string& axis, float min, float max);
template void PCLUTILS::filterPassthrough<pcl::PointNormal>(const pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_in, pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_out,const std::string& axis, float min, float max);

// 滤波--裁剪盒滤波
template<typename PointT>
void PCLUTILS::filterCropBox(const typename pcl::PointCloud<PointT>::Ptr& cloud_in, typename pcl::PointCloud<PointT>::Ptr& cloud_out, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max,bool is_negative){
    filter::cropBoxFilter<PointT>(cloud_in, cloud_out, x_min, x_max, y_min, y_max, z_min, z_max, is_negative);  // 调用filter.hpp中的函数   
}
template void PCLUTILS::filterCropBox<pcl::PointXYZ>(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_out, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max,bool is_negative);
template void PCLUTILS::filterCropBox<pcl::PointXYZI>(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_out, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max,bool is_negative);
template void PCLUTILS::filterCropBox<pcl::PointXYZRGB>(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_in, pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_out, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max,bool is_negative);
template void PCLUTILS::filterCropBox<pcl::PointNormal>(const pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_in, pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_out, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max,bool is_negative);
